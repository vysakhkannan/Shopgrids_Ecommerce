...
def checkout(request, id = None, prd_id = None):
    ...
    if request.user.is_authenticated:
        ...
        if id == '-1':
            ...
            product = Products.objects.get(id = product_id)
            try:
                cart_items = CartItems.objects.get(user = user_cart,products_id = product, buy_now = True)

            except:
                cart_items = CartItems.objects.filter(user = user_cart,products_id = product, buy_now = True).delete()
                redirect('home')
...
# placing order
@never_cache
def place_order(request, id = None, prd_id = None):
    ...
    if request.method == 'POST':
        ...
        if id == '-1':
            ...
            product_id = prd_id
            product = Products.objects.get(id = product_id)
            cart_items = CartItems.objects.get(user = ordered_user, buy_now = True, products_id = product)
            single_order = order
            user = ordered_user
            product_id = cart_items.products_id
            quantity = cart_items.quantity 
            sub_total = cart_items.sub_total
            is_active = cart_items.is_active

            OrderItems.objects.create(order = single_order, user = user, products_id = product_id, quantity = quantity, sub_total = sub_total, is_active = is_active)
            product = Products.objects.get(id = cart_items.products_id.id)
            product.stocks -= cart_items.quantity
            product.save()
            cart_items.delete()
...
def paypal_payment(request):
    ...
    new_order = Order.objects.create(user = current_user,order_id  = order_id, payment_method = body['payment_methods'], delivered_address = ordered_address, delivery_status = delivery_status, payment_status = body['status'], grand_total = body['grand_total'] )
    ...
    if cart_buy == -1:
        ...
        cart_items = CartItems.objects.get(user = current_user, buy_now = True, products_id = prd_id)        

        OrderItems.objects.create(quantity = cart_items.quantity, products_id = cart_items.products_id, sub_total = cart_items.sub_total, user = current_user, order = new_order)
...
@csrf_exempt
@login_required(login_url='login')
def buy_now(request,prd_id):

    print('buy_now',prd_id)
    ...
    try: 
        ...
        CartItems.objects.get(user = current_user, products_id = product, buy_now = True)
        return redirect('checkout',id = 1, prd_id = prd_id)
